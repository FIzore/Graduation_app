...
`qlut-secondhand-app/utils/request.ts`

调整全局响应拦截器：

* `403/400/429/500` 优先使用后端返回的 `msg` 或 `message`
* 只有后端没传文案时，才回退到默认提示
* 成功状态从仅 `200` 扩展为 `200 || 201`
* `RequestError` 抛出的内容现在也会携带后端真实错误文案
* `qlut-secondhand-app/pages/item/post.vue`

  * 发布失败时的 `403` 捕获改为展示真实风控原因
  * Toast 增强为 `duration: 3000`，方便用户看清违规提示
...
- **AI 审核态可见化**：pages/item/post.vue 的发布中提示升级为“AI 正在识别图片与校验内容...”，明确告知用户当前处于 AI 审核阶段
- **分类修正反馈**：发布成功后读取后端返回的最终分类字段，区分展示“分类已由 AI 智能修正”“已沿用您的分类”与“AI 审核通过，发布成功”三类结果提示
- **风控提示延续**：发布失败命中 403 时继续优先展示后端透传的具体风险原因，避免被通用错误文案覆盖
...
- **安全收口**：移除登录/注册接口中的明文参数日志，避免密码进入开发者工具或采集日志
- **图片兜底统一**：新增 `utils/image.ts`，统一首页、详情、搜索、我的发布、我的预约与想要列表的图片解析和缺省图逻辑
- **胶囊安全区统一**：新增 `utils/navigation.ts`，发布页、消息页、聊天室、我的发布、我的预约、我想要的页面接入微信胶囊安全区计算
- **WebSocket 状态可视化**：`utils/websocket.ts` 暴露连接状态与待发送数量，聊天室显示重连中和待发送提示，并在 `onUnload` 清理监听
- **想要闭环**：新增 `pages/profile/wishlist.vue`，个人中心展示想要数量并提供列表入口，详情收藏写入本地物品快照
- **预约闭环**：`api/item.ts` 新增 `cancelAppointment()`，我的预约页支持取消预约并刷新列表
- **列表体验修复**：我的发布、我的预约、我想要的支持下拉刷新；发布页图片上传改为按张显示进度
- **兼容性清理**：移除前端剩余 Flex `gap` 用法，修复旧版微信基础库间距风险
